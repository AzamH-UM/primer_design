...
_SUPPORTED_POLYMERASES = ['Phusion', 
...
def calculate_tm_nearest_neighbor(primer_sequence, primer_conc, polymerase):
    ...
    assert polymerase in _SUPPORTED_POLYMERASES, f'ERROR: Polymerase {polymerase} is not supported!'
    ...
    if polymerase == 'Phusion' or polymerase == 'Phusion Hot Start Flex':
        ...
        melting_temp = 1 / ((1 / Tm_base) + ((4.29 * gc_content(primer_sequence) - 3.95) * math.log(m) + 0.94 * (math.log(m)) ** 2) * 1e-05) - 273.15
